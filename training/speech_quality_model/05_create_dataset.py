...
for j in trange(len(df)):
    ...
    if len(voice_detect_output) > 0:
        ...
        if np.mean(abs(y)) <= 0.15:
            labels.append(1)  # poor quality
        else:
            labels.append(0)  # ok quality
...
df_train.to_csv("../speech_quality_model/train.csv", index=False)
print("Saved: ", "../speech_quality_model/train.csv")
df_test.to_csv("../speech_quality_model/valid.csv", index=False)
print("Saved: ", "../speech_quality_model/valid.csv")
